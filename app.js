...
function calculateManualBaselineForFutureMonths(transactions, baseMonth) {
  if (!parseMonth(baseMonth)) return { income: 0, regularExpense: 0 };
  return transactions.reduce((totals, item) => {
    if (item.isAutoGenerated || item.isAuto || monthISO(item.date) !== baseMonth) return totals;
    if (item.type === "income") {
      totals.income += item.amount;
    } else if (item.type === "expense" && item.category !== ASSET_FORMATION_CATEGORY) {
      totals.regularExpense += item.amount;
    }
    return totals;
  }, { income: 0, regularExpense: 0 });
}

function calculateProjectedRecurringExpenseForMonth(recurringExpenses, month) {
  if (!parseMonth(month)) return 0;
  return (Array.isArray(recurringExpenses) ? recurringExpenses : [])
    .filter((item) => isRecurringExpenseApplicable(item, month))
    .reduce((sum, item) => sum + item.amount, 0);
}

function calculateCumulativeBalanceImpactToAge(settings, transactions, recurringExpenses, targetAge = 60) {
  if (!settings.birthDate) return 0;
  const targetMonth = resolveWithdrawTargetMonth(settings.birthDate, targetAge);
  if (!targetMonth) return 0;

  const entryStartMonth = resolveEntryStartMonth(settings, transactions);
  if (!parseMonth(entryStartMonth) || compareMonth(entryStartMonth, targetMonth) > 0) return 0;

  const baseMonthRaw = resolveCurrentAssetTargetMonth(settings, transactions);
  const baseMonth = compareMonth(baseMonthRaw, targetMonth) > 0 ? targetMonth : baseMonthRaw;
  let total = 0;
  let month = entryStartMonth;

  while (compareMonth(month, baseMonth) <= 0) {
    const monthly = calculateMonthlySummary(transactions, settings, month);
    total += monthly.income - monthly.expense;
    month = addOneMonth(month);
  }

  if (compareMonth(baseMonth, targetMonth) >= 0) {
    return total;
  }

  const baseline = calculateManualBaselineForFutureMonths(transactions, baseMonth);
  month = addOneMonth(baseMonth);
  while (compareMonth(month, targetMonth) <= 0) {
    const projectedAutoAssetExpense = compareMonth(month, entryStartMonth) < 0
      ? 0
      : createAutoExpensesForMonth(settings, month).reduce((sum, item) => sum + item.amount, 0);
    const projectedRecurringExpense = calculateProjectedRecurringExpenseForMonth(recurringExpenses, month);
    total += baseline.income - (baseline.regularExpense + projectedRecurringExpense + projectedAutoAssetExpense);
    month = addOneMonth(month);
  }
...
function calculateAge60FinancialSummary(settings, transactions, recurringExpenses, lifeEvents) {
  const assetFormationTotalAt60 = calculateProjectedTotalAtAge(settings, 60);
  const cumulativeBalanceImpactTo60 = calculateCumulativeBalanceImpactToAge(settings, transactions, recurringExpenses, 60);
  const lifeEventTotals = calculateLifeEventTotalsThroughAge(lifeEvents, 60);
  const total = assetFormationTotalAt60
    + cumulativeBalanceImpactTo60
...
function renderDashboard(summary, settings, currentMonth, transactions, recurringExpenses, lifeEvents, expenseComposition) {
  ...
  const age60Summary = calculateAge60FinancialSummary(settings, transactions, recurringExpenses, lifeEvents);
...
function renderAssetForecast(settings) {
  ...
  const breakdownRows = [
    ...
    { label: "累積収支影響", amount: age60Summary.cumulativeBalanceImpactTo60, sign: age60Summary.cumulativeBalanceImpactTo60 >= 0 ? "+" : "−" },
