...
function buildPlannedExtraTotalsByYear(transactions, startMonth) {
  const parsedBaseMonth = parseMonth(startMonth);
  if (!parsedBaseMonth) return {};
  return (Array.isArray(transactions) ? transactions : []).reduce((map, item) => {
    ...
    if (isPlannedExtraIncome) {
      ...
      if (!parsedTargetMonth) return map;
      const monthsFromStart = ((parsedTargetMonth.year - parsedBaseMonth.year) * 12)
        + (parsedTargetMonth.monthIndex - parsedBaseMonth.monthIndex);
      if (!Number.isFinite(monthsFromStart) || monthsFromStart < 0) return map;
      year = parsedBaseMonth.year + Math.floor(monthsFromStart / 12);
    }
